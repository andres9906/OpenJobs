...
function Aceptados(props) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const data = await db.collection("Trabajos").where('Email','==',sessionStorage.getItem("email").toLowerCase()).get();
          ...
          
         {
          resultado.map(item=>( matches.small && <ListGroup.Item style={ {width: '400px'}} ><h4> {item.Nombre}</h4><br /><p>
            {item.Descripcion}<br /></p>
              <div class="but" style={{ paddingLeft: '250px', paddingTop: '30px' }}><a href={"/UpdatePu?Trabajo=".concat(item.id)}>Actualizar</a><Button variant="outline-info" onClick={() => history.push("/Search/Detail?Trabajo=".concat(item.id))}>Ver mas</Button>
              </div>
           </ListGroup.Item>))}
            {resultado.map(item=>( matches.medium && <ListGroup.Item style={ {width: '750px'}} ><h4> {item.Nombre}</h4><br /><p>
            {item.Descripcion}<br /></p>
              <div class="but" style={{ paddingLeft: '650px', paddingTop: '30px' }}><a href={"/UpdatePu?Trabajo=".concat(item.id)}>Actualizar</a><Button variant="outline-info" onClick={() => history.push("/Search/Detail?Trabajo=".concat(item.id))}>Ver mas</Button>
              </div>
           </ListGroup.Item>))}
...
export default withRouter(Aceptados);
